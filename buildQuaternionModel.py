...
class QuatDeepVONet(nn.Module):
    def __init__(self, sizeHidden=1):
        super(QuatDeepVONet, self).__init__()
...
def main():
    typeModel = "QuatDeepVONet"
    typeCriterion = "MSELoss"
    typeOptimizer = "Adam"

    if typeModel == "QuatDeepVONet":
        model = QuatDeepVONet(sizeHidden=params.BACH_SIZE).to(params.DEVICE)
    else:
        raise ValueError
